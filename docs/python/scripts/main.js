        ...
        sectionList: [
            ...
            {id: 2, title: 'Expressions', content: [
                ...
                {title: "Examples", columns: false, content: [{
                    id: 2,
                    text: "<p>Here are some examples of these operations in use, feel free to play around with the Python shell by writing some expressions yourself!</p><span>>>> 3 + 2<br>5<br>>>> 3 - 2<br>1<br>>>> 3 * 2<br>6<br>>>> 3 / 2<br>1.5<br>>>> 3 // 2<br>1<br>>>> 3 // 2 + 1 <span class='comment'># Please note that to do ceiling division, you just add one onto the result of floor division</span><br>2<br>>>> 3 ** 2<br>9<br>>>> 3 % 2<br>1</span>"
                }]},
            ...
            {id: 3, title: 'Variables', content: [
                {title: "Introduction", columns: false, content: [{
                    id: 0,
                    text: "<p>Now this is all well and good, but what if you want to use the result of your expression for later?</p><p>You can assign it to a variable like so: <code>x = 3 + 2</code>, this will create an <code>int</code> called <code>x</code> that is equal to <code>5</code>. You could then use <code>x</code> later on, or change it like so:</p><span>>>> x = 3 + 2<br>>>> x<br>5<br>>>> x = x - 2 <span class='comment'># 5 - 2</span><br>>>> x<br>3<br>>>> x *= 2 <span class='comment'># 3 * 2</span><br>>>> x<br>6</span><p>As you may have noticed, I did <code>x *= 2</code>. This is shorthand for <code>x = x * 2</code>. It will do the same thing and you can use this format for any operator.</p>"
                }]},
            ...
            {id: 4, title: 'Conditionals', content: [
                {title: "Introduction", columns: false, content: [{
                    id: 0,
                    text: "<p>What if we wanted to do different things to <code>x</code> based on what <code>x</code> is?<br>Well, we can, we can use an <code>if</code> statement.</p><p>Usage in Python looks like this:</p><span>if condition:<br><pre>    do stuff</pre></span><p><code>condition</code> in this case is a conditional, some examples being:</p><span>>>> x = 0<br>>>> x<br>0<br>>>> x == 0 <span class='comment'># x is equal to 0</span><br>True<br>>>> x > 0 <span class='comment'># x is greater than 0</span><br>False<br>>>> x <= 0: <span class='comment'># x is less than or equal to 0</span><br>True<br>>>> x != 0: <span class='comment'># x is not equal to 0</span><br>False</span>"
                }]},
                {title: "Booleans and equivalences", columns: false, content: [{
                    id: 0,
                    text: "<p>As you can see, these conditionals return True or False, this is because they are booleans. Equivalents to these will also work as conditions. If x is 1 then x will be equivalent to True meaning instead of having to write:</p><span>if x == 1:</span><p>you can write</p><span>if x: <span class='comment'># this is shorthand for if x is True</span></span><p>The opposite goes for <code>False</code></p><span>if x == 0:</span><p>to get this, we have to <b>negate</b> (invert) the previous statement, we do this by using <code>not</code></p><span>if not x:</span>"
                }]},
                {title: "Using the <code>and</code> and <code>or</code> keywords", columns: false, content: [{
                    id: 0,
                    text: "<p>If you want to have multiple conditions where they both have to be <code>True</code> you can do this</p><span>if condition1 and condition2:<br><pre>    do stuff</pre></span><p>if you want to have multiple conditions where only one has to be <code>True</code> you can do this</p><span>if condition1 or condition2:<br><pre>    do stuff</pre></span><p>it's very important that you make sure that your conditional is written correctly, sometimes you will need to use parenthesis</p><span>>>> False and True or True <span class='comment'># False and True have to be the same so they're False, making the statement False or True, so this is True</span><br>True<br>>>> False and (True or True) <span class='comment'># False and (True or True) becomes False and True, which is False</span><br>False<br>>>> (False and True) or True <span class='comment'># False and True make False, making the statement False or True, so this is true</span><br>True<br>>>> not False and True or True <span class='comment'># not False, which is True makes the statement True and True or True, which is going to be True</span><br>True<br>>>> not (False and True or True) <span class='comment'># this is a negation of the first conditional, so it is False</span><br>False</span><p>don't forget, even these statements using <code>or</code> and <code>and</code> are conditionals too.</p>"
                }]},
            ...
            // ]},
            // {id: 5, title: 'Docstrings & Formatting', content: [
            //     {title: "Introduction", columns: false, content: [{
            //         id: 0,
            //         text: " "
            //     },
